...
# 实现BERT中的交叉层，该层包含self attention和cross attention机制
class BertCrossLayer(nn.Module):
    ...
    def forward(
        ...
        # TODO:加入mamba
        # config = MambaConfig(d_model=attention_output.size(-1), n_layers=2)
        # MambaModule = Mamba(config).to(attention_output.device)
        # attention_output = MambaModule.forward_unimodal(attention_output)

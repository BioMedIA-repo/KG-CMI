...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # self.clip_transforms = keys_to_transforms(['clip_224'], size=224)
        self.clip_transform = False
        for transform_key in transform_keys:
            # if 'clip' and 'clip_224' in transform_key:
        ...
        if len(names) != 0:
            ...
            self.answer_type = self.table["answer_type"].to_pandas().tolist()

        else:
            self.all_texts = list()

        # Record Index Mappings
        self.index_mapper = dict()  # 创建一个空字典用于存储索引映射关系
    ...
    def get_image(self, index, image_key="image"):
        raw_image = self.get_raw_image(index, image_key=image_key)
        image_tensor = [tr(raw_image) for tr in self.transforms]
        # image_tensor_224 = [tr(raw_image) for tr in self.clip_transforms]

        return {
            "image": image_tensor,
            # "img_224": image_tensor_224,
    ...
    def collate(self, batch, mlm_collator):
        ...
        keys = set([key for b in batch for key in b.keys()]) #提取btach中所有样本的键合成一个集合
        dict_batch = {k: [dic[k] if k in dic else None for dic in batch] for k in keys} # 构建字典：键是原样本的键，值是该键在每个样本中的值

        img_keys = [k for k in list(dict_batch.keys()) if "image" in k or 'img_224' in k]
        img_sizes = list()
        ...
        for img_key in img_keys:
            ...
            # else:
            #     new_images = [torch.zeros(batch_size, 3, 224, 224) for _ in range(view_size)]
        ...
        txt_keys = [k for k in list(dict_batch.keys()) if "text" in k] # 筛选出文本相关键
        if len(txt_keys) != 0:
            encodings = [[d[1] for d in dict_batch[txt_key]] for txt_key in txt_keys]  # 提取每个文本键对应的编码
            flatten_encodings = [e for encoding in encodings for e in encoding] # 分别提取出list下的数据
            flatten_mlms = mlm_collator(flatten_encodings)  # 应用 MLM 整理器：生成掩码后的输入和标签

            for i, txt_key in enumerate(txt_keys):
                texts, encodings = ([d[0] for d in dict_batch[txt_key]], [d[1] for d in dict_batch[txt_key]])  # 分离文本内容和编码
                mlm_ids, mlm_labels = (

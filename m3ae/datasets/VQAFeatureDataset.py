...
class VQADataset(Dataset):
    def __init__(self, name, dataroot, device = "cuda" if torch.cuda.is_available() else "cpu"):
        ...
        self.clip_model, self.preprocess = clip.load("/data/qgking/zxy/MultimodalPromptRetrieval-main/models/ViT-B-32.pt", device=device)
    ...
    def create_retrieval_dataset(self, data_loader, is_training_phase = True, retrieval_k=15, use_additional_data=False):
    ...
    def retrieve_closest_qa_pairs(self, batch, return_ans = False, return_info = None, return_dists = False, use_quantifier=True):
        ...
        image_encoding = self.clip_model.encode_image(batch["img_224"][0])
        text_encoding = self.clip_model.encode_text(clip.tokenize(batch["text"]).to(self.device))
